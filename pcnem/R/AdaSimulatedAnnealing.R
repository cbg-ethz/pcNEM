...
nem.AdaSA <- function(n,phi,D,control){
    ...
    if(iterations > 1){
      for (z in 1:iterations){
        if(noiseEst == TRUE){ # if we allow the new edge reversal move then sample the move type
        ...
        switch(as.character(chosenmove),
               "1"={ # searching for DAG
                 ...
                 if(naccepts_DAG == stepsave){
                   ...
                   #print(Temp)
                   #print(currentAR^transformAR)
                   #print(currentDAGlogscore)
                   # Updating all
                   L1 <- c(L1,list(phi))
               ...
               "2"={
                 ...
                 if(chosenNoise == 1){ # update alpha
                   ...
                   if(revallowed==1){
                     scoreratio <- exp((proposedDAGlogscore-currentDAGlogscore)/Temp) #acceptance probability
                   } else{
                     scoreratio <- exp((proposedDAGlogscore-currentDAGlogscore)/Temp) #acceptance probability
                   }
